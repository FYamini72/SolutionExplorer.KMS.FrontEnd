...
{
    public partial class PersonnelTrainingCourseList : ComponentBase
    {
        ...
        int? SelectedId = null;
        PersonnelTrainingCourseSearchDto SearchModel = new();
        MudTable<PersonnelTrainingCourseDisplayDto> Table;
        ...
        async Task<TableData<PersonnelTrainingCourseDisplayDto>> LoadServerData(TableState state, CancellationToken token)
        {
            SearchModel.CalculateTakeAndSkip(state.Page, state.PageSize);

            var result = await Http.GetByFilterAsync<PersonnelTrainingCourseSearchDto,
                ApiResult<BaseGridDto<PersonnelTrainingCourseDisplayDto>>>(
                "api/PersonnelTrainingCourse/GetByFilter", SearchModel);

            return new()
            {
                Items = result?.Data?.Data ?? Enumerable.Empty<PersonnelTrainingCourseDisplayDto>(),
        ...
        private async Task DeleteItem(int itemId)
        {
            ...
            if (confirmResult)
            {
                var apiResult = await Http.DeleteAsync<ApiResult>($"api/PersonnelTrainingCourse?id={this.SelectedId}");
